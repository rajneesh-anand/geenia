      ...
      {({ open }) => (
        <div className="relative z-10 lg:top-[1px]">
          ...
          >
            <Listbox.Options
              static
              className="absolute ltr:right-0 rtl:left-0  lg:ltr:left-0 lg:rtl:right-0 w-full py-1 mt-1 overflow-auto bg-brand-light rounded-md shadow-dropDown max-h-60 focus:outline-none text-sm min-w-[140px]"
            >
              {options?.map((option) => (
                <Listbox.Option
                  key={option.id}
